...
##################################################
# Create additional dataframes with alternate organizations
##################################################
...
# ## Build a dataframe averaged to the plot level ignoring dates
# ## just a test and I dont think makes any logical sense, do not use without more thinking
# # I think in reality this is a useless dataframe when it comes down to my actual analysis
# # I need to work with the temporal component somehow considering how things change over time
# # Taking averages over time does not seem like it makes real sense
# # Just set this up so that I have a dataframe to write the code for a linear model off of
# plot.no.date_data <- plant_data %>%
#   group_by(plot) %>%
#   summarize(
#     num_plants = n_distinct(plant_id),
#     num_racemes = sum(num_racemes),
...
# original_df$date <- as.Date(as.character(original_df$date))
# 
# # Sort the dataframe by plant_id and date
# original_df <- original_df %>% arrange(plant_id, date)
# 
# # Calculate the interval length and identify the interval order
# original_df <- original_df %>%
#   group_by(plant_id) %>%
#   mutate(interval_order = row_number(),
#          interval_length = lead(date) - date)
# 
# # Calculate the changes over intervals for columns 6 to 11
# interval_changes <- original_df %>%
#   group_by(plant_id) %>%
#   mutate(across(starts_with("column6"), ~ lead(. - lag(.)), .names = "change_{.col}"))
# 
# # Create the final new dataframe
# new_df <- interval_changes %>%
#   select(plant_id, interval_order, interval_length, plot, starts_with("column6"), starts_with("column7")) # Include other numeric columns as needed
# 
# # Rename columns
# colnames(new_df)[4:9] <- paste("interval1_", colnames(new_df)[4:9], sep = "")
# 
# # Add "inter" prefix to interval_order column
# new_df$interval_order <- paste("inter", new_df$interval_order, sep = "")
# 
# # Print or inspect the new dataframe
# print(new_df)
# 
# 
# interval_data <- plant_data %>%
#   arrange(plots, plant_id, as.Date(date, format = "%Y-%m-%d")) %>%
#   group_by(plots, plant_id) %>%
#   mutate(
#     interval = paste0("inter", row_number()),  # Create interval names
#     interval_length = as.integer(lead(as.Date(date, format = "%m-%d")) - as.Date(date, format = "%m-%d")),  # Calculate interval length
#     across(starts_with("budding_sum", "mature_sum", "post_flowering_sum", "missing_sum", "fruiting_sum"), ~ lead(.))  # Calculate changes in numeric columns
#   ) %>%
#   ungroup() %>%
